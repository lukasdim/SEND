...
public class NodeSpecCatalogLoader {
    ...
    public NodeSpecCatalogLoader(ObjectMapper objectMapper) {
        this(objectMapper, new NodeSpecSetConfig.Builder()
    ...
    public List<NodeSpec> loadAll() {
        ...
        for (NodeSpecSetConfig config : registeredSets) {
            try {
                Resource[] resources = resolver.getResources("classpath*:" + config.resourceSubdirectory() + "/*.json");
                ...
                throw new IllegalStateException("Failed to scan node-spec directory: " + config.resourceSubdirectory(), e);
            }
        }
    ...
    private NodeSpecSet inferSet(JsonNode payload, NodeSpecSet defaultSet, Resource resource) {
        ...
        try {
            ...
            throw new IllegalStateException(
                    "Unsupported node set '" + rawSet + "' in resource: " + resource.getDescription(),
                    e);
        }
    }
}
