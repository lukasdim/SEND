...
/**
 * Normalizes the stored frontend-oriented graph document into the OCaml worker
 * payload shape.
 */
@Component
public class StrategyWorkerPayloadMapper {
    ...
    public ObjectNode toWorkerPayload(StrategyDocument strategyDocument) {
        ...
        List<ResolvedGraphEdge> resolvedEdges = strategyDocument.edges().stream()
        ...
        for (String nodeType : referencedNodeTypes) {
            NodeSpec spec = nodeCatalogService.findSpec(nodeType)
