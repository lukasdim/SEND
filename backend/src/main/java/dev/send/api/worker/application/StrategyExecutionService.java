...
/**
 * Bridge-oriented execution service. Java keeps transport validation and graph
 * storage, while OCaml owns semantic validation and execution.
 */
@Service
public class StrategyExecutionService {
    ...
    public StrategyExecutionService(
            ...
            OcamlWorkerClient ocamlWorkerClient) {
    ...
    private StrategyExecutionException mapFailure(OcamlExecutionResponse response) {
        String code = response.code() == null || response.code().isBlank()
                ? "worker_failure"
                : response.code();
        ...
        HttpStatus status = switch (code) {
            case "spec_decode_failed", "graph_validation_failed", "execution_failed", "missing_executor" ->
