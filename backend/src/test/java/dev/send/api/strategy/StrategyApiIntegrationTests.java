...
class StrategyApiIntegrationTests {
  ...
  void createsListsAndFetchesStrategiesWithHandleAwareEdges() throws Exception {
    ...
    mockMvc
        .perform(post("/api/strategies").contentType(MediaType.APPLICATION_JSON).content(payload))
    ...
    mockMvc
        .perform(get("/api/strategies/s-api-1"))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.id").value("s-api-1"))
  ...
  void exposesBundledLogicexStrategyForTesting() throws Exception {
    ...
    mockMvc
        .perform(get("/api/strategies/logicex"))
        .andExpect(status().isOk())
    ...
    mockMvc
        .perform(get("/api/strategies/aapl_buy_sell_template"))
        .andExpect(status().isOk())
  ...
  void testsCurrentGraphThroughOcamlWorkerAndReturnsFlatNodeResults() throws Exception {
    ...
    when(ocamlWorkerClient.executeGraph(any(StrategyDocument.class)))
        .thenReturn(
            new OcamlExecutionResponse(
                "ok", "execute_graph", result, null, null, java.util.List.of()));
    ...
    mockMvc
        .perform(
            post("/api/strategies/test").contentType(MediaType.APPLICATION_JSON).content(payload))
        .andExpect(status().isOk())
  ...
  void simulatesGraphThroughOcamlWorkerAndReturnsStructuredSimulationResult() throws Exception {
    ObjectNode result = objectMapper.createObjectNode();
    result.putObject("summary").put("executedDays", 2).put("finalEquity", 1035.5);
    result.putObject("portfolio").put("cash", 900.0);
    result.putObject("finalNodeValues").putObject("buy-1").put("executed", true);
    ...
    when(ocamlWorkerClient.simulateGraph(
        ...
        .thenReturn(
            new OcamlExecutionResponse(
                "ok", "simulate_graph", result, null, null, java.util.List.of()));
  ...
  void rateLimitsRepeatedSimulationRequestsForTheSameClient() throws Exception {
    ObjectNode result = objectMapper.createObjectNode();
    result.putObject("summary").put("executedDays", 1).put("finalEquity", 1000.0);
    result.putObject("portfolio").put("cash", 1000.0);
    ...
    mockMvc
        ...
        .andExpect(
            jsonPath("$.message")
                .value("Strategy simulations are limited to one run every 5 seconds."))
  ...
  void returnsGlobalSimulationBoundsForSandboxStartup() throws Exception {
    ...
    mockMvc
        .perform(get("/api/strategies/simulation-bounds"))
        .andExpect(status().isOk())
